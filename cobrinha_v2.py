...
class cobra:
    ...
    def show(self):
        for XY in self.cobra:
            pygame.draw.rect(fundo, cor_cobra, [XY[0], XY[1], tamanho, tamanho])
...
class Jogo:
    ...
    def iniciar(self):
        while self.jogando:
            ...
            if self.jogando:
                ...
                textoPlacarSombra = Texto("Pontuação:"+str(self.pontos), preto, 25)
    ...
    def perdido(self):
        while self.perdeu:
            ...
            textoPerdeuSombra = Texto("Fim de jogo!", preto, 50)
            ...
            textoPontuacaoSombra = Texto("Pontuação Final: "+str(self.pontos),preto, 30)
            ...
            pygame.draw.rect(fundo, branco, [43, 118, 139, 31])
            pygame.draw.rect(fundo, preto, [45, 120, 135, 27])
            textoContinuar = Texto("Continuar(S)", branco, 30)
            textoContinuar.show(50, 125)

            ''' Desenha o botão de sair do jogo '''
            pygame.draw.rect(fundo, branco, [188, 118, 79, 31])
            pygame.draw.rect(fundo, preto, [190, 120, 75, 27])
            textoSair = Texto("Sair(N)", branco, 30)
